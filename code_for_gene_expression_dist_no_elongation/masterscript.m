...
close all; clear;
promoter='P1-mRNALL90'; %arbitrary name, but will be included the output file name
fOn = 0.25; %fOn constant for promoter
mRNALL = 90; proteinLL = 0;
for i = 1:10 %each run does 100 iterations
     TASEPmodeling_noEl_bursty_par(promoter,i,fOn,mRNALL, proteinLL);
end;
%% Result files are saved in the folder.
%% For analysis of the par results:
%% per DNA template or per simulations
TASEPmodeling_noEl_par_analysis('P1-mRNALL90',10)

close all; clear;
promoter='P1-mRNALL90-proteinLL180'; %arbitrary name, but will be included the output file name
fOn = 0.25; %fOn constant for promoter
...
%% For analysis of the par results:
%% per DNA template or per simulations
TASEPmodeling_noEl_par_analysis('P1-mRNALL90-proteinLL180',10)


close all; clear;
promoter='P1nb-mRNALL90'; %arbitrary name, but will be included the output file name
kLoading = 1/15; mRNALL = 90; proteinLL = 0;
for i = 1:10 %each run does 100 iterations
     TASEPmodeling_noEl_nonbursty_par(promoter,i,kLoading,mRNALL, proteinLL);
end;

TASEPmodeling_noEl_par_analysis('P1nb-mRNALL90',10)
